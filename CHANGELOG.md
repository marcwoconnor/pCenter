...
### Performance
- **Lazy-loaded route pages cut initial JS bundle by 76%** (closes #67). Build was warning `chunks larger than 500 kB`: every route page — Ceph (1.7k LOC), Settings (1.3k LOC), ObjectDetail tree (2.9k LOC), NoVNC `rfb` client, chart library — was loaded on first paint of `/login`. Wrapped every route page in `React.lazy()` with a single `Suspense` boundary at the Routes level (Login stays eager — entry point, no benefit to splitting). Result: main bundle **1,150 → 273 kB** minified (**300 → 84 kB gzip**). Per-route chunks now load on demand: Ceph 47 kB, Settings 54 kB, etc. Vite's chunk graph automatically split shared heavy deps too — `rfb` (NoVNC, 254 kB) only loads when the console opens; the metrics chart library (386 kB) only on metrics-bearing pages. No more 500 kB warning.

### Fixed
- **Deb postinst restarts pcenter on upgrade** (closes #55). Previously `apt upgrade pcenter` left the service in stopped state — postinst only ran `systemctl enable`, never started or restarted. Operators saw the upgrade complete cleanly but the dashboard stayed dead until they manually started the service. Fix: standard Debian postinst pattern, branching on `$2` (empty for fresh install, set for upgrade). Fresh installs keep the existing flow (enable + wait for the operator to register the first user). Upgrades enable + restart, guarded by `is-enabled` so a deliberately-disabled service doesn't get surprise-started. Pairs with v0.1.13's graceful shutdown (#34) + sd_notify readiness (#36) — `apt upgrade pcenter` is now end-to-end clean: prerm SIGTERM → 30s WebSocket/dispatcher drain → binary swap → postinst restart → sd_notify `READY=1` only after the listener binds.
