...
- **Ceph OSD lifecycle endpoints** — `POST /api/clusters/{cluster}/nodes/{node}/ceph/osd` (create on the named node, with optional separate db_dev/wal_dev BlueStore offload, encryption, CRUSH device-class override, and osds_per_device for splitting fast NVMe), `DELETE .../ceph/osd/{osdid}?cleanup=1` (destroy + LVM cleanup so the device can be reused without manual zap), `POST .../osd/{osdid}/in`, `POST .../osd/{osdid}/out`, `POST .../osd/{osdid}/scrub?deep=1`. All route through PVE REST; create + delete return UPIDs.
- **Ceph monitor + manager endpoints** — `POST /api/clusters/{cluster}/nodes/{node}/ceph/mon` (create with optional explicit mon_address for multi-NIC nodes), `DELETE .../ceph/mon/{monid}`, `POST .../ceph/mgr` (no body — PVE picks the daemon name from hostname), `DELETE .../ceph/mgr/{mgrid}`. The DELETE handlers do NOT enforce a quorum-survival check — that guard belongs in UI confirmation dialogs since "remove the last MON to recover from a corrupt monmap" is a real recovery scenario the API must allow.
- **New top-level `/ceph` page.** Replaces the inline Ceph health panel that lived inside the Storage page. Tabs: Status (health checks, capacity, OSD/pool counts), OSDs (grouped by host with in/out/scrub/destroy actions), Pools (list with create / edit / delete dialogs), Monitors (MON + MGR + MDS lists with add/remove), Flags (noout, noscrub, etc. toggles). Cluster picker at the top is URL-driven (`/ceph?cluster=foo&tab=osds`). Auto-refreshes every 30s; manual Refresh button too. The Storage → Ceph tab now shows a quick health summary and a "Manage Ceph →" link to the new page. The legacy `/api/ceph` and `/api/ceph/command` endpoints stay for backwards compat.
